...
# seeing if there's a signif relationship between mother and daughter SR within each isofemale line
# is the variance of the daughter_ratio values within each founder_id lower than expected by random chance, with more weight given to larger brood sizes.
# 1. Bootstrap random distributions: Simulate random daughter_ratio values (ranging from 0 to 1) for each founder_id, weighted by the respective daughter_brood_size, to generate a null distribution of variance.
# 2. Calculate observed variance: For each founder line, calculate the weighted variance of the actual daughter_ratio values.
# 3. Compare observed variance to null distribution: Compare the observed weighted variance against the bootstrapped distribution of random variances. Calculate the p-value as the proportion of bootstrapped variances that are lower than the observed variance.
...
weighted_variance <- function(x, weights) {
  ...
  sum(weights * (x - w_mean)^2) / sum(weights)
}

# Step 1: Calculate observed weighted variance for each founder_id
observed_variances <- sex_ratios_MD %>%
  group_by(founder_id) %>%
  summarize(
    obs_var = weighted_variance(daughter_ratio, daughter_brood_size)
  )

# Step 2: Create a null distribution of variances by bootstrapping
set.seed(123)
bootstrap_variances <- sex_ratios_MD %>%
  group_by(founder_id) %>%
  do({
    # Number of bootstraps
    n_bootstrap <- 1000
    # Vector to store bootstrapped variances
    boot_var <- numeric(n_bootstrap)
    
    # Bootstrap loop
    for (i in 1:n_bootstrap) {
      # Simulate random daughter_ratio values between 0 and 1
      random_ratios <- runif(nrow(.), 0, 1)
      # Calculate weighted variance for this random sample
      boot_var[i] <- weighted_variance(random_ratios, .$daughter_brood_size)
    }
    
    # Return a dataframe with founder_id and all bootstrap variances
    data.frame(boot_var = boot_var)
  })

# Step 3: Calculate p-values by comparing observed variances to bootstrapped null distribution
p_values <- bootstrap_variances %>%
  group_by(founder_id) %>%
  summarize(
    # Compare the observed variance to the null distribution
    p_value = mean(boot_var < observed_variances$obs_var[observed_variances$founder_id == unique(founder_id)])
  )

# Step 4: Merge p-values back into the main dataframe
sex_ratios_MD <- sex_ratios_MD %>%
  left_join(p_values, by = "founder_id") %>%
  mutate(significant = ifelse(p_value < 0.05, "significant", "not significant"))

# Step 5: Plot the data with significance stars above each panel
p <- ggplot(sex_ratios_MD, aes(daughter_ratio, daughter_ratio, colour = significant)) +
  geom_point(aes(size = daughter_brood_size), alpha = 0.7) +
  theme_classic() +
  ...
  xlab("Proportion of male offspring (mother)") +
  ylab("Proportion of male offspring (daughter)") +
  scale_colour_manual(values = c("significant" = "red3", "not significant" = "black")) +
  theme(
...
ggsave(file="/Users/robertbaird/documents/writing/Ling_SR/Figure_bits/mother_vs_daughter_by_line.svg", plot=p, width=12.5, height=6)#

## signif similarities between broods within isofemale lines in 18 of 28 lines
...
sex_ratios_all <- within(sex_ratios_all, generation[generation == "G0"] <- 'F0')
